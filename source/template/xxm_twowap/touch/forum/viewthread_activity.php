	...
	<div class="xxm-at-form mtm mbm">
		...
		<table cellpadding="0" cellspacing="1" border="0">
			...
			<tr>
				...
				<td><!--{if $activity['starttimeto']}-->{lang activity_start_between}<!--{else}-->$activity[starttimefrom]<!--{/if}--></td>
			</tr>
			<tr>
			...
			<tr>
				<th>{lang gender}: </th>
				<td><!--{if $activity['gender'] == 1}-->{lang male}<!--{elseif $activity['gender'] == 2}-->{lang female}<!--{else}-->{lang unlimited}<!--{/if}--></td>
			</tr>
			<!--{if $activity['cost']}-->
			<tr>
			...
			<!--{if !$_G['forum_thread']['is_archived']}-->
				...
				<!--{if $applied && $isverified < 2}-->
					<!--{if !$isverified}-->
					<tr><td colspan="2" class="grey xxmfst">{lang activity_wait}</td></tr>
					<!--{else}-->
					<tr><td colspan="2" class="grey xxmfst">{lang activity_join_audit}</td></tr>
					<!--{/if}-->
					<!--{if !$activityclose}--><!--{/if}-->
...
<div class="xxmclear cl"></div>
<!--{if !$activityclose}-->
	<!--{if $_G['uid']}-->
		<!--{if !$activityclose && (!$applied || $isverified == 2)}-->
		<div id="activityjoin" class="xxm-at-form mtw mbw">
			...
			<form name="activity" id="activity" method="post" autocomplete="off" action="forum.php?mod=misc&action=activityapplies&fid=$_G[fid]&tid=$_G[tid]&pid=$post[pid]{if $_GET['from']}&from=$_GET['from']{/if}&mobile=2" >
				...
				<table cellpadding="0" cellspacing="1" border="0">
				<div class="rq hm mbn"><strong class="fszes">&#25105;&#35201;&#25253;&#21517;</strong></div>
				<!--{if $_G['setting']['activitycredit'] && $activity['credit'] && !$applied}-->
				<div colspan="2" class="grey xxmfst hm mbn">{lang activity_need_credit} <strong class="xxmfst rq">$activity[credit]</strong> {$_G['setting']['extcredits'][$_G['setting']['activitycredit']][title]}</div>
				<!--{/if}-->
				<!--{if $activity['cost']}-->
				<tr>
		...
		<div id="activityjoincancel" class="xxm-at-form mtw mbw">
			<table cellpadding="0" cellspacing="1" border="0">
				<div class="rq hm mbn"><strong class="fszes">{lang activity_join_cancel}</strong></div>
	...
	<!--{else}-->
		<div class="mtw mbw btn-big">
			<a href="javascript:popup.open('{lang nologin_tip}', 'confirm', 'member.php?mod=logging&action=login');" class="touch">&#25105;&#35201;&#25253;&#21517;</a>
		</div>
	<!--{/if}-->
<!--{else}-->
...
<div id="postmessage_$post[pid]" class="postmessage">
	<h1 class="activitytitle xxmbb1 fszes">{lang activity}&#20171;&#32461;</h1>
	<div class="cl">$post[message]</div>
</div>
