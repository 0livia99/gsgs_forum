...
<header class="header">
    <div class="nav">
   	 	<a href="<!--{if $_GET[action] == 'newthread'}-->forum.php?mod=forumdisplay&fid=$_G[fid]&page=$_GET[page]<!--{else}-->forum.php?mod=redirect&goto=findpost&ptid=$_G[tid]&pid=$pid<!--{/if}-->" class="z xxmfont iconzuo xxmfstt"></a>
		...
		<span><!--{if $_GET[action] == 'newthread'}-->{lang send_threads}<!--{elseif $_GET[action] == 'reply'}-->{lang join_thread}<!--{elseif $_GET[action] == 'edit'}-->{lang edit}<!--{else}-->{lang send_threads}<!--{/if}--></span>
    </div>
</header>
<!-- header end -->
...
<div class="wp">
	<div class="post_from">
		<ul class="cl">
			...
			<li class="bl_line">
				{lang post_delpost} ?&nbsp;&nbsp;<input type="checkbox" name="delete" id="delete" class="xxm-checkbox" value="1" title="{lang post_delpost}{if $thread[special] == 3}{lang reward_price_back}{/if}">
			</li>
			<!--{/if}-->
			
			<li class="bl_line">
				<span>{lang modcp_forum} : </span><a href="forum.php?mod=forumdisplay&fid=$_G[fid]" class="blue">$_G['forum'][name]</a>
			</li>
...
<script type="text/javascript" src="{$_G['style']['styleimgdir']}/images/buildfileupload.js?{VERHASH}"></script>
