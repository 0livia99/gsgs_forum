...
<span><?php if($_GET['action'] == 'newthread') { ?>发帖<?php } elseif($_GET['action'] == 'reply') { ?>回复<?php } elseif($_GET['action'] == 'edit') { ?>编辑<?php } else { ?>发帖<?php } ?></span>
    </div>
</header>
<!-- header end -->
...
<li class="bl_line">
删除本帖 ?&nbsp;&nbsp;<input type="checkbox" name="delete" id="delete" class="xxm-checkbox" value="1" title="删除本帖<?php if($thread['special'] == 3) { ?>，返还悬赏费用，不退还手续费<?php } ?>">
</li>
<?php } ?>

<li class="bl_line">
<span>当前版块 : </span><a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $_G['fid'];?>" class="blue"><?php echo $_G['forum']['name'];?></a>
</li>

<?php if($_GET['action'] != 'reply') { ?>
<li>
<input type="text" tabindex="1" class="post_input" id="needsubject" size="30" autocomplete="off" value="<?php echo $postinfo['subject'];?>" name="subject" placeholder="标题" fwin="login">
</li>
<?php } else { ?>
<li class="bl_line">
回复: <?php echo $thread['subject'];?>
...
<li class="area">
<textarea class="pt" id="needmessage" tabindex="3" autocomplete="off" id="<?php echo $editorid;?>_textarea" name="<?php echo $editor['textarea'];?>" cols="80" rows="10"  placeholder="内容" fwin="reply"><?php echo $postinfo['message'];?></textarea>
</li>
...
<label id="extra_rushreplyset_b" onclick="showExtra('extra_rushreplyset')" style="display: none;"><span id="extra_rushreplyset_chk">抢楼主题</span></label>
...
<p class="ptm">
<em class="xxmfst grey">单次回帖奖励: <?php echo $_G['setting']['extcredits'][$extcreditstype]['unit'];?><?php echo $_G['setting']['extcredits'][$extcreditstype]['title'];?> (留空或填 0 为不奖励)</em>
...
<label>
每人最多可获得 
<select id="replycredit_membertimes" name="replycredit_membertimes" class="ps vm"><?php for($i=1;$i<11;$i++) {;?><option value="<?php echo $i;?>"<?php if($replycredit_rule['membertimes'] == $i) { ?> selected="selected"<?php } ?>><?php echo $i;?></option><?php };?></select> 次
</label>, 
<label>
中奖率 
...
<p class="ptm grey">阅读权限按由高到低排列，高于或等于选中组的用户才可以阅读</p>
<p class="ptm grey">
...
<p class="ptm">
<em class="xxmfst grey">售价 :  <?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit'];?><?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title'];?>(最高 <?php echo $_G['group']['maxprice'];?> <?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit'];?>)</em>
<input type="text" id="price" name="price" class="xxm-p-input" value="<?php echo $thread['pricedisplay'];?>" />
<em class="xxmfst grey"><?php if($_G['group']['maxprice'] && ($_GET['action'] == 'newthread' || $_GET['action'] == 'edit' && $isfirstpost)) { if($_G['setting']['maxincperthread']) { ?>主题出售最高收入上限为 <?php echo $_G['setting']['maxincperthread'];?> <?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit'];?>&nbsp;&nbsp;&nbsp;&nbsp;<?php } if($_G['setting']['maxchargespan']) { ?>主题最多能销售 <?php echo $_G['setting']['maxchargespan'];?> 个小时<?php if($_GET['action'] == 'edit' && $freechargehours) { ?>，本主题还能销售 <?php echo $freechargehours;?> 个小时<?php } } } ?></em>
</p>
</div>
<?php } if($_G['group']['allowposttag']) { ?>
<div id="extra_tag_c" class="exfm cl" style="display: none;">
<p class="ptm">
<em class="xxmfst grey">标签 : 用逗号或空格隔开多个标签，最多可填写 5 个</em>
...
<p>
<label for="rushreply" class="grey"><input type="checkbox" name="rushreply" id="rushreply" class="pc vm" value="1" <?php if($_GET['action'] == 'edit' && getstatus($thread['status'], 3)) { ?>disabled="disabled" checked="checked"<?php } ?> onclick="extraCheck(3)" /> 设为抢楼主题</label>
</p>
<p class="ptm">
<em class="xxmfst grey">抢楼时间:</em>
...
<em class="xxmfst grey">多楼层用英文逗号隔开,*号可匹配任意数或空值,如:8,88,*88</em>
</p>
</div>
<div class="sadd">
<p class="ptm">
<em class="xxmfst grey">回帖限制 : 每个用户回帖次数上限</em>
...
<p class="ptm">
<label for="stopfloor" class="xxmfst grey">截止楼层:</label>
...
<p class="ptm">
<label for="creditlimit" class="xxmfst grey"><?php if($_G['setting']['creditstransextra']['11']) { ?><?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['11']]['title'];?><?php } else { ?>积分<?php } ?>下限 : <?php if($_G['setting']['creditstransextra']['11']) { ?>(<?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['11']]['title'];?>)<?php } else { ?>总积分<?php } ?>大于此设置才能参与抢楼，可不填</label>
...
<div class="ptm cl">
<p><em class="xxmfst grey">基本属性</em></p>
<p class="xxm-label">
...
<div class="ptm cl">
<p><em class="xxmfst grey">文本特性</em></p>
<p class="xxm-label">
...
<div class="ptm cl">
<p><em class="xxmfst grey">管理操作</em></p>
<p class="xxm-label">
